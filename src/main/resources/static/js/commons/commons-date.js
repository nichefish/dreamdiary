...
commons.date = (function() {
    return {
        ...
        strToDate: function(dateStr) {
            ...
            const normalizedDateStr = commons.date.normalizeDateStr(dateStr);
            // `Date` 객체 생성 및 반환
            try {
         ...
         * @param {string} [pattern] - 날짜 형식 패턴 (선택적). 기본값은 `commons.date.ptnDate`.
         * @returns {string|undefined} - 변환된 날짜 문자열 또는 유효하지 않은 경우 `undefined`.
         */
        dateToStr: function(date, pattern) {
            if (!(date instanceof Date)) return;

            pattern = pattern || commons.date.ptnDate;      // 패턴이 없을 경우 기본 패턴 설정
        ...
        asDate: function(date) {
            ...
            if (commons.util.isEmpty(date)) return undefined;
        ...
        getCurrYyStr: function() {
            return commons.date.getCurrYy().toString();
        },
        ...
        getCurrMnthStr: function(dev = false) {
            return commons.date.getCurrMnth(dev).toString();
        },
        ...
        getDateAddDayStr: function(date, day, pattern) {
            ...
            const newDate = commons.date.getDateAddDay(date, day);
            if (newDate === null) return null;

            pattern = pattern || commons.date.ptnDate;      // 패턴이 없을 경우 기본 패턴 설정

            return commons.date.dateToStr(newDate, pattern);
        },
        ...
        getDateStrAddDay: function(datestr, day) {
            // 유효한 날짜인지 확인
            const date = commons.date.asDate(datestr);
        ...
        getDateStrAddDayStr: function(datestr, day, pattern) {
            // 유효한 날짜인지 확인
            const newDate = commons.date.getDateStrAddDay(datestr, day);
            if (newDate === null) return null;
        ...
        getCurrTimeStr: function() {
            const currDate = new Date();
            return commons.date.getTimeStr(currDate);
        },

        /**
         * 현재 날짜와 시간을 `yyyy-MM-dd HH:mm:ss` 형식의 문자열로 반환합니다.
         * @param {string} [pattern] - 날짜-시간 형식 (선택적). 기본값은 `commons.date.ptnDatetime`.
         ...
         * @param {string} [pattern] - 반환할 날짜 형식 (선택적). 기본값은 `commons.date.ptnDate`.
         * @returns {string|null} - 변환된 날짜 문자열 또는 유효하지 않은 경우 `null`.
         */
        getDateStrStr: function(dateStr, pattern) {
            // 날짜 형식이 8자리인 경우 `YYYY-MM-DD`로 변환
            const date = commons.date.strToDate(dateStr);
            if (date === null) return null;
         ...
         * @param {string} startDt - 시작 날짜 문자열.
        ...
        getCurrDateWithinChk: function(startDt, endDt) {
            ...
            return commons.date.getDateWithinChk(currentDateStr, startDt, endDt);
        },
        ...
        getDateWithinChk: function(dt, startDt, endDt) {
            // 입력 값들을 Date 객체로 변환
            const dateToCheck = commons.date.asDate(dt);
            const startDate = commons.date.asDate(startDt);
            const endDate = commons.date.asDate(endDt);

            // 시작 날짜가 비어있을 경우 종료 날짜만 체크
            if (commons.util.isEmpty(startDt)) {
                return dateToCheck <= endDate;
            }
            // 종료 날짜가 비어있을 경우 시작 날짜만 체크
            if (commons.util.isEmpty(endDt)) {
        ...
        getDayweekStr: function(date, locale) {
            ...
            if (date === null) return null; // 유효하지 않은 날짜 처리

            const dw = date.getDay(); // 요일 인덱스
        ...
        getWeekdayDate: function(date, paramWeekday) {
            ...
            const currentDay = date.getDay(); // 현재 요일 인덱스
            const dayDiff = date.getDate() - currentDay + (currentDay === 0 ? -7 : 0) + paramWeekday;
        ...
        getWeekdayDateStr: function(date, paramWeekday, pattern) {
            ...
            // 특정 요일의 날짜 객체를 가져오기
            const weekdayDate = commons.date.getWeekdayDate(date, paramWeekday);
            if (weekdayDate === null) return null; // 유효하지 않은 경우 null 반환

            // 날짜 객체를 지정된 패턴으로 변환하여 문자열 반환
            return commons.date.dateToStr(weekdayDate, pattern);
        },
        ...
        isBefore: function(beginSelector, endSelector) {
            // 시작일과 종료일의 값 가져오기
            const begin = document.querySelector(beginSelector)?.value;
            ...
            // 유효한 날짜인지 확인
            if (beginCompareDate === null || endCompareDate === null) return null;
        ...
        navigateDate: function(searchType, stdrdDate, type) {
            // 기준일을 Date 객체로 변환
            const date = commons.date.asDate(stdrdDate);
            if (date === null) return null;

            switch(searchType) {
                case "day":
                    // 어제, 내일
                    return commons.date.getDateStrAddDay(stdrdDate, (type === "prev") ? -1 : 1);
                case "week":
                    // 이전주, 다음주
                    return commons.date.getDateStrAddDay(stdrdDate, (type === "prev") ? -7 : 7);
                case "month":
                    // 이전월, 다음월
                    if (type === "prev") {
        ...
        navigateDateStr: function(searchType, stdrdDate, type, pattern) {
            ...
            const newDate = commons.date.navigateDate(searchType, stdrdDate, type);
            if (newDate === null) return null;
