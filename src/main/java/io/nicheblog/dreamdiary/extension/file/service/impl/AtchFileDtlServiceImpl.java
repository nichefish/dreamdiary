...
public class AtchFileDtlServiceImpl
    ...
    public void addFiles(final MultipartHttpServletRequest multiRequest, final List<AtchFileDtlEntity> atchFileList) throws Exception {
        ...
        while (fileNmIterator.hasNext()) {
            ...
            final String orgnFileNm = Optional.ofNullable(multipartFile.getOriginalFilename())
                    .orElse(DateUtils.getCurrDateStr(DatePtn.DATE));
            final String orgnFileExtn = orgnFileNm.substring(orgnFileNm.lastIndexOf('.') + 1);
            final String replaceFileNm = System.nanoTime() + "." + orgnFileExtn;
            // 실제 파일 저장
            // multipartfile.transferto(file)를 사용시 절대경로를 사용하지 않으면 문제 발생!
            final Path abPath = Paths.get(fileUploadPath + replaceFileNm)
                    .toAbsolutePath();
            final String abPathStr = Paths.get(fileUploadPath)
                    .toAbsolutePath()
                    .toString();
