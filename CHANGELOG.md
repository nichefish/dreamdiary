...
- 개선 사항: 
  - Flyway 플러그인 도입.
    - DB 스키마 버전 관리 및 변경 관리 체계 구축.
  - 저널 일자, 일기, 꿈에 모달 히스토리 기능 적용.
    - 상세, 등록/수정 모달 조회 및 처리 후 이전 모달이 있을 경우 되돌아가도록 처리.
  - config 파일 패키지 구조 변경.
  - scss 파일 분리. MessageBundle 정리. 주석 강화.
  - 환경변수 적용 시점에 appilcation.yml 못 읽는 문제 해결 위해 파일에서 프로퍼티 강제 파싱 적용.
- 오류 수정:
