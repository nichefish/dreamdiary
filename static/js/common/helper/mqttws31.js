...
cF.mqttws31 = (function () {
    return {
        ...
        start: function (componentId, options, onSuccFunc) {
            ...
            const mqttClient = cF.mqttws31.initClient(componentId, options);
        ...
        initClient: function (componentId, options) {
            ...
            mqttClient.onConnectionLost = function () {
                ...
                if (options.connRetryCnt < 5) {
                    if (options.reconnTimer)
                        clearTimeout(options.reconnTimer);
        ...
        connect: function (mqttClient, options = {}, onSuccFunc) {
            ...
            mqttClient.connect({
                ...
                onSuccess: function () {
                    if (isLogging)
                        console.log(mqttClient.clientId + " // connect: onConnect...");
                    if (topics.length > 0) {
                ...
                onFailure: function () {
                    if (isLogging)
                        console.log(mqttClient.clientId + " // connect: onFailure...");
                }
            });
        },
        /**
        ...
        publish: function (mqttClient, options, topic, payloadStr) {
            ...
            try {
                ...
                if (isLogging)
                    console.log(mqttClient.clientId + " // message sent:: " + topic + " // " + payloadStr);
            }
            catch (error) {
                console.error("Failed to publish message:", error);
                if (isLogging)
                    console.log(mqttClient.clientId + " // message publish failed: " + error.message);
            }
        }
    };
})();
