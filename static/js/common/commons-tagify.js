...
commons.tagify = (function () {
    ...
    return {
        ...
        initWithCtgr: function (selector, tagCtgrMap, additionalOptions = {}) {
            ...
            if (!tagify.categoryInputContainer || !tagify.tagCtgrInput)
                return tagify;
            ...
            tagify.tagCtgrInput.addEventListener('keydown', function (event) {
                ...
                if (event.key === 'Escape') {
                    ...
                    Array.from(tags).forEach(existingTagElmt => {
                        ...
                        if (existingTagData.value !== newValue)
                            return;
                        const existingCtgr = existingTagData.data ? existingTagData.data.ctgr : "";
                        if (existingCtgr === newCtgr)
                            tagify.removeTags(existingTagElmt); // 마킹된 태그 제거
                    });
                    // 새 태그 추가
